...
export default function () {
  ...
  return (
    <AppContainer>
      <FlatList
        contentContainerStyle={tw.style(`min-h-screen pb-40`)}
        refreshControl={
          <RefreshControl
          ...
          <>
            ...
            <View
              ...
              <View style={tw`flex-row justify-between my-3 ltr:bg-gray-400`}>
                <View>
                  <Text style={tw`${textColor}`}>{trans('categories')}</Text>
                </View>
                <Pressable
                  onPress={() =>
          ...
          <>
            <View
              ...
              <View style={tw`flex-row justify-between my-3`}>
                <View>
                  <Text style={tw`${textColor}`}>{trans('products')}</Text>
                </View>
                <Pressable
                  onPress={() =>
            ...
            <View
              style={tw`w-full bg-white dark:bg-transparent p-2 rounded-md mb-10`}>
              <Text style={tw`${textColor} my-3 text-lg`}>
                {trans('categories')}
              </Text>
              <FlatList
                columnWrapperStyle={tw`w-full justify-start items-center my-2`}
                showsVerticalScrollIndicator={false}
                showsHorizontalScrollIndicator={false}
                horizontal={false}
                numColumns={3}
                ...
                renderItem={({item}) => (
                  <Pressable
                    style={tw`flex flex-col w-1/3 justify-start ml-1`}
                    key={item.id}>
                    <FastImage
                      source={{uri: getThumb(item.image)}}
                      style={tw`w-25 h-25 ml-2 rounded-md`}
